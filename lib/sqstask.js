...
exports = module.exports = function(AWSConfig) {
  ...
  if (
    !AWSConfig ||
    typeof AWSConfig.accessKeyId === 'undefined' ||
    typeof AWSConfig.secretAccessKey === 'undefined' ||
    typeof AWSConfig.awsAccountId === 'undefined'
  ) {
    throw new Error(
      'You must initialize sqstask module with a configuration object that has properly \
defined accessKeyId, secretAccessKey and awsAccountID fields, coresponding to core AWS credentials'
  ...
  if (
    typeof AWSConfig.AttributeName !== 'undefined' &&
    typeof AWSConfig.AttributeValue !== 'undefined'
  ) {
...
exports.put = function(topic, message, callback) {
  ...
  var body = pack(message),
    options = { QueueName: queueName, MessageBody: body },
  ...
  sqs.SendMessage(options, function sendMessageToQueueCallback(err, data) {
    ...
    if (err) {
      ...
      typeof data.Body.SendMessageResponse === 'undefined' ||
      typeof data.Body.SendMessageResponse.SendMessageResult === 'undefined' ||
      typeof data.Body.SendMessageResponse.SendMessageResult
...
exports.get = function(topic, skipPolling, callback) {
  ...
  sqs.ReceiveMessage(options, function getMessageFromQueueCallback(err, data) {
    ...
    if (
      typeof data.Body.ReceiveMessageResponse !== 'undefined' &&
      typeof data.Body.ReceiveMessageResponse.ReceiveMessageResult !==
        'undefined' &&
      typeof data.Body.ReceiveMessageResponse.ReceiveMessageResult.Message !==
        'undefined'
    ) {
      if (
        typeof data.Body.ReceiveMessageResponse.ReceiveMessageResult.Message
        ...
        message = unpack(
          data.Body.ReceiveMessageResponse.ReceiveMessageResult.Message.Body
        );
      }
...
exports.topicEnsureExists = function(topic, callback) {
  ...
  queueExists(name, function ensureQueueCheckCallback(err) {
    if (err) {
      // Probably doesn't exist, let's try to create
...
exports.Message.prototype.del = function(callback) {
  if (!this.topic) {
    callback(new Error('Cannot delete a message with empty topic.'));
    return;
  }
  if (!this.id) {
    callback(new Error('Cannot delete a message with empty ID.'));
    return;
  }
...
function unpack(bodyString) {
  try {
    ...
    return new Error(
      'Error parsing JSON message from the queue. Probably invalid JSON: ' +
        bodyString
    );
  }
}
